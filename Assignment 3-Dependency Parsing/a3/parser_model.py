...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embeddings = nn.Parameter(torch.tensor(embeddings))  # 感觉是E (V, d)
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.Tensor(self.embed_size * self.n_features, self.hidden_size))  # W:(d*m, h)
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.Tensor(self.hidden_size))  # b1:(1, h)
        ...
        # torch.tensor是根据已有数据建立tensor, torch.Tensor根据形状
        ### END YOUR CODE
    ...
    def embedding_lookup(self, w):
        ...
        assert x.shape[0] == w.shape[0], "x's shape[0]: {:} should be batch_size, expected {:}".format(x.shape[0], w.shape[0])
        assert x.shape[1] == w.shape[1] * self.embed_size, \
            "x's shape[1]: {:} should be n_features * embed_size, expected {:}".format(x.shape[1], w.shape[1] * self.embed_size)
        
        
        # 题干对这一函数的测试其实并不完整，所以加了assert
        # flatten和view的区别: flatten指定维，以某一维为起止进行flatten;view指定变化后的形状(可用-1代替自动算的维度) 
        # ps:stackoverflow上讲view(-1)要更快些，但相差不大
        # 操作是先把w降为一维(w和pdf中的w一致，用于从E中选取word); E是embedding matrix(self.embeddings), 选取直接通过index_select可得;
        # 随后要再拼接成pdf中x的形式 (1, d*m)
    ...
    def forward(self, w):
        ...
        assert logits.shape[0] == w.shape[0], \
            "logits's shape[0]: {:} should be batch_size, expected {:}".format(logits.shape[0], w.shape[0])
        assert logits.shape[1] == self.n_classes, \
            "logits's shape[1]: {:} should be n_classes, expected {:}".format(logits.shape[1], self.n_classes)            
