...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0 :
        ...
        transitions = model.predict(minibatch) # 这里的predict是针对minibatch的每一项parse进行一次下一步操作的预测
        for parse, transition in zip(minibatch, transitions):
            parse.parse_step(transition) # 这么写应该最符合pdf的要求     
            #parse.parse([transition]) #这种也能过, 看parse的函数, 传的参数是一个列表, 如果不加[]会造成死循环
            if len(parse.stack) == 1 and len(parse.buffer) == 0:  # finish collecting a PartialParse's dependencies
