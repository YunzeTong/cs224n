...
if args.function == 'pretrain':
    ...
    tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3,
             ...
             num_workers=4)
    mytrainer = trainer.Trainer(model=mymodel, train_dataset=pretrain_dataset, test_dataset=None, config=tconf)
    mytrainer.train()
    torch.save(mymodel.state_dict(), args.writing_params_path)
    ...
    if args.reading_params_path is not None:
        max_epochs_used = 10
        mymodel.load_state_dict(torch.load(args.reading_params_path))
    # Goal 2
    train_dataset = dataset.NameDataset(pretrain_dataset,  
                                        open(args.finetune_corpus_path, 'r', encoding='UTF-8').read())
    tconf = trainer.TrainerConfig(max_epochs=max_epochs_used, batch_size=256, learning_rate=6e-4,
             lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
             num_workers=4)
    mytrainer = trainer.Trainer(model=mymodel, train_dataset=train_dataset, test_dataset=None, config=tconf)
    mytrainer.train()
    # Goal 3
    torch.save(mymodel.state_dict(), args.writing_params_path)
    ...
    with open(args.outputs_path, 'w') as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path)):
            ...
            x, mymodel = x.to(1), mymodel.to(1)  # my add
