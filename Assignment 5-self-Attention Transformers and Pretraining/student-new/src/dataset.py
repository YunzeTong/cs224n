...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]  # 0: get a line sequence in wiki.txt accordding to idx(line number)
        doc = doc[0:min(random.randint(4, int(self.block_size*7/8)), len(doc))] # 1: truncate from beginning, itimate eg, not sure?
        # 2: gauss distribution, not sure if it is good because the std will make effect
        mask_len = int(random.normalvariate(len(doc) / 4, 2))
        while mask_len >= len(doc) - 2:
            mask_len = int(random.normalvariate(len(doc) / 4, 2))
        remain_len = len(doc) - mask_len
        prefix_len = random.randint(1, remain_len)
        prefix = doc[0:prefix_len]
        masked_content = doc[prefix_len:prefix_len+mask_len]
        suffix = doc[prefix_len+mask_len:]
        # 3
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        corruption_dataset = CharCorruptionDataset(open('wiki.txt', 'r', encoding='utf-8').read(), 128) 
